...
public class Scorch extends MobEffect {
    ...
    public boolean applyEffectTick(@NotNull LivingEntity entity, int amplifier) {
        if (!entity.level().isClientSide()) {
            ...
            if (ticks >= TICKS_PER_STACK) {
                ...
                // 获取当前效果的持续时间
                MobEffectInstance currentEffect = entity.getEffect(FELEffects.SCORCH);
                int duration = currentEffect != null ? currentEffect.getDuration() : 200;

                // 更新效果等级
                entity.addEffect(new MobEffectInstance(FELEffects.SCORCH, duration, newLevel, false, true, true));
    ...
    private static void triggerIgnite(LivingEntity entity, int level) {
        ...
        entity.addEffect(new MobEffectInstance(FELEffects.IGNITE, 1, Math.max(level, 0)));
    ...
    public static void onEntityTick(EntityTickEvent.Post event) {
        if (event.getEntity() instanceof LivingEntity entity && !entity.level().isClientSide()) {
            ...
            for (MobEffectInstance inst : entity.getActiveEffects()) {
                if (inst.getEffect().value() instanceof Scorch) {
                    hasEffect = true;
                    break;
                }
            }
