...
/**
 * 电光充能：缓慢叠加层数并在满层时消耗全部层数，召唤一道闪电攻击敌人
 * <p>
 * 机制：
 * <ol>
...
public class BoltCharge extends MobEffect {
    ...
    public static void onPlayerLogin(PlayerEvent.PlayerLoggedInEvent event) {
        ...
        if (player.hasEffect(FELEffects.BOLT_CHARGE)) {
    ...
    public static void syncToClient(LivingEntity entity, int chargeCount) {
        if (entity instanceof ServerPlayer serverPlayer) {
            PacketDistributor.sendToPlayer(serverPlayer, new BoltChargeSyncPacket(entity.getUUID(), chargeCount));
        }
    }
    ...
    public static void onLivingAttack(LivingDamageEvent.Pre event) {
        ...
        // 检查充能是否已满
        int currentCharges = BoltChargeAPI.getChargeCount(livingAttacker);
        if (currentCharges >= BoltChargeAPI.MAX_CHARGES) {
            // 触发闪电打击，消耗所有充能
            BoltChargeAPI.triggerLightning(event.getEntity(), livingAttacker);
        }
    }
    ...
    public static void onEffectRemoved(MobEffectEvent.Remove event) {
        if (event.getEffect() == FELEffects.BOLT_CHARGE) {
    ...
    public static void onEffectExpired(MobEffectEvent.Expired event) {
        ...
        if (instance != null && instance.getEffect() == FELEffects.BOLT_CHARGE) {
            clearCharges(event.getEntity());
        }
    }
